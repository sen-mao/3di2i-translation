...
class Style2Layer(nn.Module):
    def __init__(self, 
        ...
        **unused_kwargs):
        ...
        if w_dim > 0:  # w_dim = 512, generator
            self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)  # affine [in=512, out=60]
            self.weight = torch.nn.Parameter(
               torch.randn([out_channels, in_channels, 1, 1]).to(memory_format=memory_format))  # [in=60, out=128]
            self.bias = torch.nn.Parameter(torch.zeros([out_channels]))  # [128]
        
        else:  # discriminator
            self.weight = torch.nn.Parameter(torch.Tensor(out_channels, in_channels))  # [out=64, in=112]
            self.bias = torch.nn.Parameter(torch.Tensor(out_channels))  # [64]
            self.weight_gain = 1.
    ...
    def forward(self, x, w=None, fused_modconv=None, gain=1, up=1, **unused_kwargs):
        ...
        if self.w_dim > 0:           # modulated convolution
            ...
            if x.ndim == 2:  # MLP mode
                ...
                x = F.relu(F.conv2d(x, self.weight[:,:,None, None], self.bias))  # self.weight: [64, 64], relu(x*weight+bias)
...
class NeRFBlock(nn.Module):
    ...
    def __init__(self, nerf_kwargs):
        ...
        if self.positional_encoding == 'gauss':
            ...
            dim_embed = D * self.n_freq_posenc * 2  # 3*10*2 = 60
            dim_embed_view = 3 * self.n_freq_posenc_views * 2  # 24
        ...
        if self.tcnn_backend:
            ...
            self.fc_in  = Style2Layer(dim_embed, self.hidden_size, w_dim, activation=self.activation)  # Style2Layer(in_feature=60, out_feature=128, style=512, (affine): FullyConnectedLayer())
            self.num_ws = 1
            ...
            if not (self.merge_sigma_feat or self.no_sigma):
                self.sigma_out = ToRGBLayer(self.hidden_size, self.shuffle_factor, w_dim, kernel_size=1)  # ToRGBLayer((affine): FullyConnectedLayer())
                self.num_ws += 1
    ...
    def forward_nerf(self, option, p_in, ray_d=None, ws=None, z_shape=None, z_app=None):
        ...
        net = self.fc_in(p, ws[:, 0] if ws is not None else None)  # todo: fc_in ?
        if self.n_blocks > 1:
        ...
        net = torch.cat(feat_inputs, 1) if len(feat_inputs) > 1 else net  # [8, 64, 32, 32]
        ...
        if self.predict_rgb:
            ...
            feat_out = torch.cat([rgb, feat_out], 1)  # [8, 67, 32, 32]
        ...
        if feat_out.ndim == 2:  # mlp mode
            ...
            feat_out  = rearrange(feat_out,  '(b s) d h w -> b (h w s) d', s=n_steps)  # [8, 67, 32, 32] -> [2, 4096, 67]
...
class CameraRay(object):
    ...
    def get_camera(self, batch_size, device, mode='random', fov=None, force_uniform=False):
        if fov is not None:
            ...
            camera_matrix = self.camera_matrix  # [1, 4, 4]
        camera_mat = camera_matrix.repeat(batch_size, 1, 1).to(device)  # [batch_size, 4, 4]
        ...
        elif isinstance(mode, torch.Tensor):  # world_mat: [batch_size, 4, 4]
    ...
    def get_pixel_coords(self, tgt_res, camera_matrices, corner_aligned=True, margin=0, theta=None, invert_y=True):
        ...
        if (theta is not None):  # 绕其起点沿逆时针方向旋转theta
...
class VolumeRenderer(object):
    ...
    def forward_rendering_with_grid(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        if pre_p_target < 1:  # use density grid to prune samples
            ...
            pre_topp_asg2 = torch.cat([torch.arange(pre_topp_lens[b], device=p_i.device) +
    ...
    def forward_rendering(self, H, output, fg_nerf, nerf_input_cams, nerf_input_feats, latent_codes, styles):
        ...
        if self.hierarchical and (not H.get('disable_hierarchical', False)):
            ...
            fg_weights, bg_lambda = self.C.calc_volume_weights(  # todo ?
                sigma, di if self.C.dists_normalized else di_trs,  # use real dists for computing weights, 
                ray_vector, last_dist=0 if not H.fg_inf_depth else 1e10)[:2]
    ...
    def forward_rendering_background(self, H, output, bg_nerf, nerf_input_cams, latent_codes, styles_bg):
        ...
        feat      = rearrange(feat, 'b (n s) d -> b n s d', s=H.n_bg_steps)  # -> [batch_size, 1024, 4, 67]
        sigma_raw = rearrange(sigma_raw.squeeze(-1), 'b (n s) -> b n s', s=H.n_bg_steps)  # -> [2, 1024, 4]
    ...
    def post_process_outputs(self, outputs, freeze_nerf=False, freeze_img_nerf=False):
        if freeze_nerf:
            # outputs = [x.detach() if isinstance(x, torch.Tensor) else x for x in outputs]
            if freeze_img_nerf == False:
                outputs[0].detach()
            elif freeze_img_nerf == True:
                outputs = [x.detach() if isinstance(x, torch.Tensor) else x for x in outputs]
...
class NeRFSynthesisNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **block_kwargs,            # Other arguments for SynthesisBlock.
        ...
        if self.resolution_start < self.resolution_vol:  # resolution_start for finetuning
        ...
        self.fg_nerf = NeRFBlock(foreground_kwargs)  # fg_nerf
        ...
        if not self.V.no_background:
            background_kwargs.update(dict(
                ...
                activation=self.activation))
            self.bg_nerf = NeRFBlock(background_kwargs)  # bg_nerf
            self.num_ws += self.bg_nerf.num_ws
        else:
            self.bg_nerf = None

        self.num_ws_nerf = self.num_ws
        ...
        # upsampler
        upsamplers = self.U.build_network(w_dim, self.fg_nerf.rgb_out_dim, **block_kwargs)  # todo: b64, b128, b256
        if len(upsamplers) > 0:
        ...
        self.freeze_nerf = freeze_nerf  # FreezeNeRF
        self.freeze_img_nerf = freeze_img_nerf  # if freeze img_nerf
        self.steps = None
    ...
    def forward(self, ws_nerf, ws, **block_kwargs):
        block_ws, imgs, rand_imgs = [], [], []
        batch_size = block_kwargs['batch_size'] = ws_nerf.size(0)
        ...
        if "camera_matrices" not in block_kwargs:
            if 'camera_mode' in block_kwargs:
                block_kwargs["camera_matrices"] = self.get_camera(batch_size, device=ws_nerf.device, mode=block_kwargs["camera_mode"])
            else:
        ...
        if "latent_codes" not in block_kwargs:
            block_kwargs["latent_codes"] = self.get_latent_codes(batch_size, device=ws_nerf.device)

        # deal with roll in cameras
        block_kwargs['theta'] = self.C.get_roll(ws_nerf, self.training, **block_kwargs)
        ...
        with torch.autograd.profiler.record_function('prepare_nerf_path'):
            ...
            if self.fg_nerf.num_ws > 0:  # use style vector instead of latent codes?
                block_kwargs["styles"] = ws_nerf[:, :self.fg_nerf.num_ws]
        ...
        with torch.autograd.profiler.record_function('nerf'):
            ...
            x, img, _ = self.V.post_process_outputs(outputs['full_out'], self.freeze_nerf, self.freeze_img_nerf)  # FreezeNeRF
            ...
            if 'rand_out' in outputs:
                x_rand, img_rand, rand_probs = self.V.post_process_outputs(outputs['rand_out'], self.freeze_nerf)   # FreezeNeRF
            ...
            if img is not None:
                ...
                block_kwargs['x_nerf'] = x
                # block_kwargs['x_nerf'] = x + torch.normal(mean=0., std=0.01, size=x.shape).to(x.device)  # add noise for x_nerf
        ...
        if (cur_resolution > self.resolution_vol) or self.progressive_nerf_only:
            ...
            with torch.autograd.profiler.record_function('upsampling'):
                ...
                imgs, xs  = self.U.forward_network(blocks, block_ws, x, img, target_res, self.alpha, **block_kwargs)
                img      = imgs[-1]
                x        = xs[-1]
                if len(rand_imgs) > 0:   # nerf path regularization
                    rand_imgs, rand_xs = self.U.forward_network(blocks, block_ws, x_rand, img_rand, target_res, self.alpha, skip_up=True, **block_kwargs)
        ...
        block_kwargs['x'] = x
        if 'step' in self.init_kwargs and self.init_kwargs.step == 'step2':
            block_kwargs['imgs'] = imgs
            block_kwargs['xs'] = xs
        return block_kwargs
    ...
    def forward_adapted(self, ws_nerf, ws, fake_x_nerf, **block_kwargs):
        n_levels, end_l, _, target_res = self.get_current_resolution()
        # batch_size = block_kwargs['batch_size'] = ws[1].size(1)
        cur_resolution = self.resolution_start
        ...
        if "camera_matrices" not in block_kwargs:
            if 'camera_mode' in block_kwargs:
                block_kwargs["camera_matrices"] = self.get_camera(batch_size, device=ws[0].device,
        ...
        if "latent_codes" not in block_kwargs:
            block_kwargs["latent_codes"] = self.get_latent_codes(batch_size, device=ws[0].device)

        # deal with roll in cameras
        block_kwargs['theta'] = self.C.get_roll(ws[0], self.training, **block_kwargs)
        '''
        # -------------------- #
        ...
        if (cur_resolution > self.resolution_vol) or self.progressive_nerf_only:
            ...
            with torch.autograd.profiler.record_function('upsampling'):
                ...
                block_ws = self.U.forward_ws_split(ws, blocks)
                img = None
                imgs, xs = self.U.forward_network(blocks, block_ws, fake_x_nerf, img, target_res, self.alpha, **block_kwargs)
                img = imgs[-1]
                x = xs[-1]
        block_kwargs['adapted_img'] = img
        block_kwargs['adapted_x'] = x
        if 'step' in self.init_kwargs and self.init_kwargs.step == 'step2':
            block_kwargs['adapted_imgs'] = imgs
            block_kwargs['adapted_xs'] = xs
        return block_kwargs
...
class Discriminator(torch.nn.Module):
    def __init__(self,
        ...
        **unused
        ...
        # final output module
        if self.c_dim > 0:
            # DiscriminatorEpilogueNoMappingnetwork do not use mappingnetwork
            self.b4 = DiscriminatorEpilogueNoMappingnetwork(channels_dict[4], self.c_dim, cmap_dim=cmap_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
        elif self.c_dim == 0:
    ...
    def forward(self, inputs, c=None, aug_pipe=None, return_camera=False, **block_kwargs):
        step = -1
        if 'step' in block_kwargs: # for step2
            step = block_kwargs['step']
            del block_kwargs['step']
        ...
        for res in block_resolutions:
            ...
            if (lowres_head == res) and (self.alpha > -1) and (self.alpha < 1) and (alpha > 0):
                if self.architecture == 'skip':
                    img = img * alpha + img0 * (1 - alpha)  # todo:alpha
                if self.progressive:
                    x = x * alpha + block.fromrgb(img0) * (1 - alpha)
            x, img = block(x, img, **block_kwargs)
            # step2 only need return x outputted by b64 module.
            if step == 2 and f'b{res}' == 'b64':
                break
        if step == 2 and f'b{res}' == 'b64':
            return x
        
        # predict camera based on discriminator features
        if self.predict_camera and (c.size(-1) == 0) and (not self.separate_camera):
