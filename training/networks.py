...
def modulated_conv2d(
    ...
    **unused,
    ...
    if not fused_modconv:
        x = x * styles.to(x.dtype).reshape(batch_size, *rsizes)  # todo: x*styles ?
        if mode == '2d':
...
class FullyConnectedLayer(torch.nn.Module):
    def __init__(self,
        ...
        # ---------- #
        # self.weight = torch.nn.Parameter(torch.randn([out_features, in_features]) / lr_multiplier)  # [60, 512]
        # self.bias = torch.nn.Parameter(torch.full([out_features], np.float32(bias_init))) if bias else None  # [60]
        # ---------- #
    ...
    def forward(self, x):
        w = self.weight.to(x.dtype) * self.weight_gain  # [512, 512]
        b = self.bias  # [512]
        if b is not None:
            b = b.to(x.dtype)
            if self.bias_gain != 1:
        ...
        if self.activation == 'linear' and b is not None:
            x = torch.addmm(b.unsqueeze(0), x, w.t())  # x×w.t()+b
        else:
            x = x.matmul(w.t())  # x*w: [2, 512]
            x = bias_act.bias_act(x, b, act=self.activation)  # lrelu(x+b): [2, 512]
        return x
...
class Conv2dLayer(torch.nn.Module):
    ...
    def forward(self, x, gain=1):
        w = self.weight * self.weight_gain  # [3, 64, 1, 1]
        b = self.bias.to(x.dtype) if self.bias is not None else None  # [3]
...
class MappingNetwork(torch.nn.Module):
    def __init__(self,
        ...
        **unused,
        ...
        trainable = True if (c_dim == 0 and freeze_nerf_mappingnetwork == False) or \
                            (c_dim > 0 and freeze_synthesisblock_mappingnetwork == False) \
                         else False
    ...
    def forward(self, z=None, c=None, truncation_psi=1, truncation_cutoff=None, skip_w_avg_update=False, styles=None, **unused_kwargs):
        ...
        if self.w_avg_beta is not None and self.training and not skip_w_avg_update:
            with torch.autograd.profiler.record_function('update_w_avg'):
                self.w_avg.copy_(x.detach().mean(dim=0).lerp(self.w_avg, self.w_avg_beta))  # todo: x.detach()

        # Broadcast.
        if self.num_ws is not None:
            with torch.autograd.profiler.record_function('broadcast'):
                x = x.unsqueeze(1).repeat([1, self.num_ws, 1])  # [batch_size, 512] -> [batch_size, 17, 512]
        ...
        **unused_kwargs
        ...
        if up == 2:
            if 'pixelshuffle' in upsample_mode:
                ...
                self.adapter = torch.nn.Sequential(
                    Conv2dLayer(out_channels * 2, out_channels // 4, kernel_size=1, activation=activation),  # todo: out_channels, out_channels // 4
                    # Conv2dLayer(out_channels, out_channels // 4, kernel_size=1, activation=activation),  # (512, 128)
                    Conv2dLayer(out_channels // 4, out_channels, kernel_size=1, activation='linear'),  # todo: out_channels // 4, out_channels * 4
                    # Conv2dLayer(out_channels // 4, out_channels * 4, kernel_size=1, activation='linear'),  # (128, 2048)
                ) 
        ...
        self.act_gain = bias_act.activation_funcs[activation].def_gain
        self.affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)  # todo: conv0.affine.in_channels = 64
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.num_ws_nerf = self.synthesis.num_ws_nerf
        self.num_ws  = self.synthesis.num_ws - self.synthesis.num_ws_nerf
        self.mapping = None
        self.encoder = None

        if len(mapping_kwargs) > 0:   # Use mapping network with uncondional (i.e. c_dim=0) for for nerf.
            self.mapping_nerf = util.construct_class_by_name(
                class_name=mapping_kwargs.get('module_name', "training.networks.MappingNetwork"),
                z_dim=z_dim, c_dim=0, w_dim=w_dim, num_ws=self.num_ws_nerf, **mapping_kwargs)

        if len(mapping_kwargs) > 0:   # Use mapping network with conditional for synthesisblock.
        ...
        if len(encoder_kwargs) > 0:   # Use Image-Encoder
            encoder_kwargs['model_kwargs'].update({'num_ws_nerf': self.num_ws_nerf, 'num_ws': self.num_ws, 'w_dim': self.w_dim})
    ...
    def forward(self, z=None, c=None, styles=None, truncation_psi=1, truncation_cutoff=None, img=None, **synthesis_kwargs):
        if styles is None:
            ...
            if (self.encoder is not None) and (img is not None):  #TODO: debug
                ...
                ws_nerf = self.mapping_nerf(z, c=None, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
                ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
        else:
            ws = styles

        img = self.synthesis(ws_nerf, ws, **synthesis_kwargs)
        return img
    ...
    def forward_adapted(self, z=None, c=None, styles=None, truncation_psi=1, truncation_cutoff=None, fake_x_nerf=None, **synthesis_kwargs):
        if styles is None:
            assert z is not None
            ws_nerf = self.mapping_nerf(z, c=None, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)  # todo: ws_nerf of forward_adapted
            ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
        else:
            ws_nerf, ws = styles
        img = self.synthesis.forward_adapted(ws_nerf, ws, fake_x_nerf, **synthesis_kwargs)
        return img

    def get_final_output_adapted(self, *args, **kwargs):
        img = self.forward_adapted(*args, **kwargs)
        if isinstance(img, list):
            return img[-1]
        elif isinstance(img, dict):
            return img['adapted_img']
        return img

    def get_inter_adapted(self, z=None, c=None, styles=None, truncation_psi=1, truncation_cutoff=None, fake_x_nerf=None, alpha=None, **synthesis_kwargs):
        # source ws
        sws_nerf = self.mapping_nerf(z, c=None, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
        sws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, **synthesis_kwargs)
        # target ws
        tws_nerf, tws = styles
        # ws
        ws_nerf, ws = sws_nerf * alpha + tws_nerf * (1-alpha), sws * alpha + tws * (1-alpha)
...
class DiscriminatorEpilogue(torch.nn.Module):
    def __init__(self,
        ...
        self.conv = Conv2dLayer(in_channels + mbstd_num_channels, in_channels, kernel_size=3, activation=activation, conv_clamp=conv_clamp)  # todo: conv
    ...
    def forward(self, x, img, cmap, force_fp32=False):
        ...
        if self.mbstd is not None:
            x = self.mbstd(x)
        x = self.conv(x)  # (512, 4, 4) -> (512, 4, 4)
        x = self.fc(x.flatten(1))  # (512, 4, 4) -> (512)
...
@persistence.persistent_class
class DiscriminatorEpilogueNoMappingnetwork(torch.nn.Module):
    def __init__(self,
        in_channels,                    # Number of input channels.
        c_dim,
        cmap_dim,                       # Dimensionality of mapped conditioning label, 0 = no label.
        ...
        # self.out = FullyConnectedLayer(in_channels, final_channels if cmap_dim == 0 else cmap_dim)
        # ---------- #
        self.conv = Conv2dLayer(in_channels + mbstd_num_channels, in_channels, kernel_size=4, conv_clamp=conv_clamp, padding=0)
        self.final = Conv2dLayer(in_channels, c_dim, kernel_size=1, conv_clamp=conv_clamp)
        # ---------- #


    def forward(self, x, img, cmap=None, c=None, force_fp32=False):
        misc.assert_shape(x, [None, self.in_channels, self.resolution, self.resolution]) # [NCHW]
        _ = force_fp32 # unused
        dtype = torch.float32
        memory_format = torch.contiguous_format
        ...
        if self.mbstd is not None:
            x = self.mbstd(x)
        x = self.conv(x)  # (513, 4, 4) -> (512, 1, 1)
        x = self.final(x) # (512, 1, 1) -> (3, 1, 1)
