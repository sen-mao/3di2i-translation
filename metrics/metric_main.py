...
#----------------------------------------------------------------------------
def calc_metric_trans(metric, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    assert is_valid_metric(metric)
    opts = metric_utils.MetricOptions_Trans(**kwargs)
...
@register_metric
def fid50k_transmix(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False)
    fid = frechet_inception_distance.compute_fid_transmix(opts, max_real=None, num_gen=50000)
    return dict(fid50k_transmix=fid)
