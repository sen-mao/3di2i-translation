...
def get_camera_pose(range_u, range_v, range_r, val_u=0.5, val_v=0.5, val_r=0.5,
    ...
    loc = loc * radius.unsqueeze(-1)  # [batch_size, 3]
    R = look_at(loc)  # [batch_size, 3, 3]
    RT = torch.eye(4).reshape(1, 4, 4).repeat(batch_size, 1, 1)  # [batch_size, 4, 4]
    RT[:, :3, :3] = R
    RT[:, :3, -1] = loc
...
def to_sphere(u, v, angular=False):
    ...
    if not angular:
        ...
        theta, phi = u, v  # 0<=theta<=2pi, 0<=phi<=pi
