...
@click.option('--outdir', help='Where to save the output images', type=str, required=True, metavar='DIR', default='./results_inter/afhq')
...
# ws_nerf, ws
@click.option('--w_path', help='w path', type=str, default=None)
@click.option('--alpha', help='alpha: 0~1', type=float, default=-1)
...
def generate_images(
    ...
    with torch.no_grad():
        ...
        if save_3dframes:
            if alpha != -1:
                curr_out_dir = os.path.join(outdir, f'seed{seed}_{batch_idx}_alpha{alpha}')
            else:
    ...
    if w_path:
        # ws_nerf and ws from .pth
        w = torch.load(w_path)
        ws_nerf, ws = w['ws_nerf'][None,:,:].repeat(batch_size, 1, 1).to(device), w['ws'].repeat(batch_size, 1, 1).to(device)
    z_ada = torch.from_numpy(np.random.RandomState(seed+2).randn(batch_size, G.z_dim)).to(device)
    z_ada = z
    ...
    with torch.no_grad():
        ...
        for gen_img in tqdm(imgs, desc='step2', ncols=80):
            ...
            for i in range(c_len):
                ...
                if ws_nerf is not None and ws is not None and alpha==-1:
                    adapted_img = G.get_final_output_adapted(styles=[ws_nerf, ws], fake_x_nerf=fake_x_nerf, noise_mode='const')
                elif ws_nerf is not None and ws is not None and 0 <= alpha <= 1:
                    adapted_img = G.get_inter_adapted(z=z_ada, c=label, styles=[ws_nerf, ws], fake_x_nerf=fake_x_nerf, alpha=alpha, noise_mode='const')
                else:
